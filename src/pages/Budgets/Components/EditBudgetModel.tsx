...
const EditBudgetModal: React.FC<EditBudgetModalProps> = ({
    ...
    const fetchCategories = async () => {
      if (user) {
        ...
        if(fetchedCategories.data) {
          fetchedCategories.data = fetchedCategories.data.filter((category) => category.type === 'EXPENSE');
  ...
  const handleCategoryAllocation = (categoryId: string, newAmount: number) => {
    ...
    if (potentialTotal > formData.amount) {
      setError(`Cannot allocate more than the total budget amount (${formatCurrency(formData.amount, user?.preferences.currency || "USD")})`);
      return;
    }
  ...
  const handleBudgetAmountChange = (newAmount: number) => {
    if (newAmount < totalAllocated) {
      setError("Cannot set budget amount lower than current allocations. Please reduce category allocations first.");
      return;
    }
    setError(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (totalAllocated > formData.amount) {
      setError("Total allocations exceed budget amount. Please adjust allocations.");
      return;
    }
  ...
  return (
    <Dialog isOpen={isOpen} onClose={onClose} title="Edit Budget">
      <form onSubmit={handleSubmit} className="space-y-6">
        {error && (
          <Alert variant="error" title="Failed to update budget" icon={<AlertCircle className="w-5 h-5" />}>
            {error}
          </Alert>
        )}
            ...
            <div className="space-y-1">
              <div className="relative">
                <Input
                  ...
                  onChange={(e) => handleBudgetAmountChange(parseFloat(e.target.value))}
                  required
                  ...
                  >
                    ...
                    <div className="relative w-32">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleCategoryAllocation(category.id, parseFloat(e.target.value) || 0)}
                      />
                    </div>
                  </div>
                );
